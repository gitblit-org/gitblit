 ...
 *
 */
public abstract class TicketServiceTest extends RepoUnitTest {
	...
	protected IStoredSettings getSettings(boolean deleteAll) throws Exception {
		File dir = new File(GitBlitTestConfig.REPOSITORIES, getRepository().name);
		if (deleteAll) {
			if (dir.exists()) {
				FileUtils.delete(dir, FileUtils.RECURSIVE | FileUtils.RETRY);
			}
			JGitUtils.createRepository(GitBlitTestConfig.REPOSITORIES, getRepository().name).close();
		}
