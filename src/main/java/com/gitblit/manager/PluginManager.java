...
public class PluginManager extends DefaultPluginManager implements IPluginManager {
	...
	public boolean refreshRegistry() {
		String url = runtimeManager.getSettings().getString(Keys.plugins.registry, "http://gitblit.github.io/gitblit-registry/plugins.json");
		try {
			return download(url);
		} catch (Exception e) {
	...
	protected boolean download(String url) throws IOException {
		...
		File tmpFile = new File(pFolder, StringUtils.getSHA1(url) + ".zip");
		if (tmpFile.exists()) {
			tmpFile.delete();
		}
