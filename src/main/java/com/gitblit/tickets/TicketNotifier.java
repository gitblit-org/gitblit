		...
		if (lastChange.hasPatchset() && ticket.isOpen()) {
			...
			if (diffstat != null) {
				...
				for (PathChangeModel path : diffstat.paths) {
					String add = "";
					if (path.insertions > 0) {
						MessageFormat.format(addPattern, path.insertions);
					}
					String del = "";
					if (path.deletions > 0) {
						del = MessageFormat.format(delPattern, path.deletions);
					}
					String diff = null;
					...
					case MODIFY:
						if (add.length() > 0) {
							// insertions & maybe deletions
							diff = add + (del.length() > 0 ? ("/" + del) : "");
						} else {
							// just deletions
							diff = del;
						}
